...
def preguntar_instrucciones():
    ...
    # Si la decisión es "s" entonces se imprime en pantalla las instrucciones.
        ...
        En cambio, si el abono es útil esto agregará mucha agua y nutrientes a
        todas las plantas.
...
def menu_interactivo(turno, agua=[], nutrientes=[], plantas=0,
    ...
    # Si el turno es distinto de cero, entonces el juego puede comenzar.
        ...
        if revisar_muerte(agua) is True or revisar_muerte(nutrientes) is True:
            ...
            ronda_bonus = preguntar_si_o_no("¿Desea jugar de nuevo? (s/n): ")
...
def ronda_mala_suerte(lista_comida):
    ...
    # todos los elementos.
        ...
        # restar vida con valores entre 5 y 10.
        else:
            lista_comida[0] = restar_vida(lista_comida[0], 5, 10)
...
def agregar_comida_aux(num_planta, lista_comida, contador):
    ...
    # a la planta que se necesita.
        ...
        if num_planta == contador:
            # A la planta se le sumará en la comida un número entre 1 y 5
            # (ambos inclusive).
            lista_comida[0] += randint(1, 5)
...
# TODO: NO sabemos si la voy a usar todavía
def encontrar_menor(lista_comida, elemento):
    if lista_comida == []:
        return elemento
    else:
        if lista_comida[0] < elemento:
            return encontrar_menor(lista_comida[1:], lista_comida[0])
        else:
            return encontrar_menor(lista_comida[1:], elemento)
